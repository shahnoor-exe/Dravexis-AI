...
export const LandingPage: React.FC = () => {
  ...
  return (
    <div className="landing-container flex flex-col h-screen w-full bg-cyber-obsidian text-zinc-100 relative overflow-x-hidden overflow-y-auto">
      {/* Backgrounds */}
      ...
      {/* Main Content */}
      <div className="relative z-20 flex flex-col items-center min-h-screen px-4 md:px-12 pt-16 pb-12 w-full max-w-7xl mx-auto">
        
        {/* Hero Section */}
        <div ref={heroRef} className="flex flex-col items-center text-center max-w-4xl opacity-0">
          <div className="text-neon-cyan text-[10px] md:text-xs font-mono uppercase tracking-[0.3em] mb-4 border border-neon-cyan/20 px-4 py-1.5 rounded bg-neon-cyan/5">
          ...
          {isUnavailable ? (
            <>
              ...
              <button 
                ...
                className="px-6 py-3 border border-zinc-600 hover:border-zinc-400 bg-zinc-800/50 hover:bg-zinc-700/50 text-white font-mono text-sm uppercase tracking-wider rounded transition-all"
              >
                Retry Connection
              </button>
            </>
            ...
            <>
              <button 
                ...
                className="px-8 py-3 border border-neon-cyan/80 bg-neon-cyan/10 hover:bg-neon-cyan/20 text-neon-cyan hover:text-white font-mono text-sm uppercase tracking-wider rounded backdrop-blur transition-all duration-300 hover:shadow-[0_0_20px_rgba(32,227,255,0.4)] group relative overflow-hidden"
              >
              ...
              <button 
                ...
                className="px-8 py-3 border border-zinc-700 hover:border-neon-cyan/50 bg-zinc-900/50 hover:bg-zinc-800/80 text-zinc-300 hover:text-neon-cyan font-mono text-sm uppercase tracking-wider rounded backdrop-blur transition-all"
              >
...
function CapabilityPill({ label, status, detail }: { label: string, status: string, detail?: string }) {
  ...
  return (
    <div className={`flex flex-col px-3 py-1.5 border rounded-sm ${color} backdrop-blur min-w-[120px] text-center group relative cursor-help`}>
      ...
      {detail && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-[200px] bg-cyber-obsidian border border-zinc-700 p-2 rounded-sm text-[9px] text-zinc-300 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 shadow-xl pointer-events-none">
          {detail}
        </div>
      )}
    </div>
